...
def findPath(mat, lookup, max_row,max_col,max_len):
	i = max_row
	j = max_col
	chain = []
	chain.append(mat[i][j])
	print(chain)
	while(max_len!=0):
		if lookup[i-1][j] == lookup[i][j]-1:
			chain.append(mat[i-1][j])
			i = i-1
			max_len-=1
		elif lookup[i][j-1] == lookup[i][j]-1:
			chain.append(mat[i][j-1])
			j = j-1
			max_len-=1
	print(chain)
...
def findSnakeSequence(mat):
	...
	for i in range(M):
		for j in range(N):
			if (i or j):
				if (i>0 and abs(mat[i-1][j] - mat[i][j])==1 ):
				...
				if (j>0 and abs(mat[i][j-1] - mat[i][j])== 1):
					lookup[i][j] = max(lookup[i][j] ,lookup[i][j-1]+1)
					if max_len < lookup[i][j]:
						max_len = lookup[i][j]
						max_row = i
						max_col = j
	print_dd_arr(lookup, M,N)
	print (max_len)
	print (max_row)
	print (max_col)
	findPath(mat,lookup,max_row,max_col,max_len)
...
# 	path = findPath(lookup, mat, max_row, max_col)print_dd_arr(lookup ,M, N)
#
